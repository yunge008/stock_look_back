...
投入资金年化回报率按期末总盈亏与最大实际资金占用估算，年化分母只累计有未平仓 lot 的持仓交易日，空仓等待期不计入；它不等同于总账户 CAGR，需结合最大资金占用一并查看。Lot 持仓时间按买入成交日至卖出成交日的自然日差计算；未平仓 Lot 截止回测最后交易日。 质量过滤逐笔网格的回撤图按当日未平仓 Lot 的总成本计算，空仓日不计入，不受闲置现金和策略资金池上限影响。
