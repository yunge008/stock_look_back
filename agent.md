...
不要把 AkShare、回测或 SQLite 逻辑搬到浏览器。Vercel 只部署前端静态站点；公网后端地址通过 `VITE_API_BASE_URL` 注入。

## 目标买点功能`r`n`r`n- 接口：`POST /api/v1/target-entry`。`r`n- 使用最新可用日线的收盘价、过去 `lookback_days` 的最高收盘和 `ma_window` 均线。`r`n- `target_buy_price` 必须是回撤阈值价和 MA 阈值价的较低者，不能取平均或较高者。`r`n- 仅供决策辅助，不会创建订单或写入回测记录。`r`n`r`n## 关键策略规则
...
- 2026-07-14：默认资金池改为 100 万、首仓 2 万、质量确认默认勾选、360 日回撤/MA120，并加入一手资金自动补足规则。`r`n- 2026-07-14：新增最新目标买点接口和页面卡片；目标价取回撤价与 MA 阈值价的较低者。
